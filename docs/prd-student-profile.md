...
| **SP-3** ✅ | `StudentProfileScreen` — header (guardian tap-to-call), WholePictureCard, year/30/90/custom range, five lazy panels (query paused until opened, one open at a time); five entry points (Roster button, absentee row, homework + assignment check rows, class-test drill); new `MiniCharts` (`MiniStackedBars` for the CORRECT/PARTIAL/WRONG mix + `MiniLineChart` for monthly presence, both dependency-free with a legend so colour is never the only cue); 54 new STR pairs | **BUILT** — app tsc clean; expo web export exit 0 (full log checked, not `| tail`). **Live drive still owed** on the dev site: one full-view class teacher, one narrowed subject teacher, one Principal |
| **SP-4** ✅ | `GET /pdf/student-profile/:studentId?from=&to=` + the screen’s প্রোফাইল প্রিন্ট button (web-only, `PDF_SUPPORTED`) | **BUILT** — 15 new jest tests over the pure sheet body (identity, panels, narrowing note, printed-by stamp, honest empty states). **A rendered PDF still owed** — eyeball one before promoting |
