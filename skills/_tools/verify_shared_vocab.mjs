...
check("kinds are exactly the 8 phase-1 kinds + 2 library kinds + 1 class-test kind + 1 vocab kind + 1 student-comment kind + 6 classroom-observation kinds + 1 finance kind + 2 saturday-revision kinds + 1 homework-chase kind + 1 assignment-chase kind + 3 homework-confirm kinds + 2 print kinds + 2 ct-question kinds + 2 ct-result kinds + 1 upcoming-class-test kind (D-#472) (D-#72/#74/#84/#122; VC-4 += VOCAB_RESULT, D-#154; CM-2 += STUDENT_COMMENT, D-#172; CO-3 += OBSERVATION_*; CO-8 += OBSERVATION_READY_TO_PUBLISH, D-#271; FIN-2B += FINANCE_FEE_DUE, D-#227; SR-2 += SR_ABSENT/SR_DIGEST, D-#244; HW per-chase += HW_CHASE, D-#260; AS-T4 per-chase += ASSIGNMENT_CHASE, D-#88/#94; HW pending-confirm += HW_PENDING_REMINDER/HW_PENDING_ESCALATION; auto-issue += HW_AUTO_ISSUED, D-#314; PQ-5 += PRINT_DELIVERED, D-#281; web-push += PRINT_REQUESTED, D-#296; CT question loop += CT_QUESTION_REVIEW/CT_QUESTION_OFFICE, D-#342; CT-8 submit/approve loop += CT_RESULT_SUBMITTED/CT_RESULT_PUBLISHED; MR-6 += MONTHLY_REPORT; weekly digest += HW_WEEKLY_DIGEST, D-#452; guardian work claim += WORK_CLAIM_FILED/WORK_CLAIM_ESCALATED/WORK_CLAIM_RESOLVED, D-#551..#554; RL-2 += STUDENT_RETURNED, D-#556; 08:00 overdue digest += CLASS_TEST_OVERDUE_DIGEST, D-#603; syllabus publish/sign-off += EXAM_SYLLABUS_PUBLISHED/EXAM_SYLLABUS_AWAITING_PUBLISH, D-#644; resubmission hand-back += HW_RESUBMIT_ISSUED, D-#682; observer assignment += OBSERVATION_ASSIGNED)", eq(V.NOTIFICATION_KINDS, ["BELL_REMINDER","ATTENDANCE_REMINDER","CLASS_NOTE_PROMPT","CLASS_NOTE_ESCALATION","CLASS_NOTE_PUBLISHED","HW_PARENT_COMMS","HW_CHASE","HW_RESUBMIT_ISSUED","ASSIGNMENT_CHASE","REVIEW_ASSIGNED","COVER_ASSIGNED","LIBRARY_DUE_SOON","LIBRARY_OVERDUE","CLASS_TEST_RESULT","CLASS_TEST_UPCOMING","CLASS_TEST_OVERDUE_DIGEST","VOCAB_RESULT","STUDENT_COMMENT","MONTHLY_REPORT","OBSERVATION_RELEASED","OBSERVATION_RESPONSE_REMINDER","OBSERVATION_ESCALATED","OBSERVATION_RESPONDED","OBSERVATION_ASSIGNED","OBSERVATION_READY_TO_PUBLISH","FINANCE_FEE_DUE","SR_ABSENT","SR_DIGEST","HW_PENDING_REMINDER","HW_PENDING_ESCALATION","HW_AUTO_ISSUED","CT_QUESTION_REVIEW","CT_QUESTION_OFFICE","CT_RESULT_SUBMITTED","CT_RESULT_PUBLISHED","PRINT_DELIVERED","PRINT_REQUESTED","STAFF_LEAVE_SUBMITTED","HW_WEEKLY_DIGEST","TEACHING_NOTE_PUBLISHED","TEACHING_NOTE_COMMENT","TEACHING_NOTE_COMMENT_ADDRESSED","WORK_CLAIM_FILED","WORK_CLAIM_ESCALATED","WORK_CLAIM_RESOLVED","STUDENT_RETURNED","EXAM_SYLLABUS_PUBLISHED","EXAM_SYLLABUS_AWAITING_PUBLISH"]));
...
check("syllabus.{published,awaitingPublish}.* template keys registered (title + body each — MT registry, D-#131/#643)",
  ["syllabus.published.title","syllabus.published.body","syllabus.awaitingPublish.title","syllabus.awaitingPublish.body"].every((k) => V.MESSAGE_TEMPLATE_KEYS.includes(k) && V.MESSAGE_TEMPLATE_REGISTRY[k]));
