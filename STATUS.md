...
## Now / next
- **Built (Comments — any teacher can comment on any student + "my comments", server + app, D-#263) [branch
  `feat/comments-any-teacher` off dev → PR into dev]:** from live testing (a teacher was blocked by "You don't
  have write permission for this section"). Comment AUTHORING is no longer section-write-scoped: `record` drops
  `assertCanWrite` (keeps `tracker:write` + still server-resolves/stores the section); `edit` stays author-only;
  `deliver` allows the author unconditionally, else section write-scope. New `myStudentComments(studentId?)`
  (`tracker:read`, own-only, child-name joined) → a **"My comments"** list on the Comments hub + merged into the
  entry screen so an out-of-scope author can re-open/edit/deliver. Section-timeline reads stay scoped (no peeking
  at other teachers' comments). **No new perm/enum/vocab/contract; firewall untouched.** **Gate GREEN (executed):**
  verifier PASS, shared build + server tsc + app tsc clean, **jest 1492/1492 (91 suites)**, expo web export green.
  Highest D-#=#263. Not verified live.
