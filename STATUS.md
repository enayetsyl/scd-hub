...
- 2026-07-22 (cont. 15): **BUILT — English Drive editor → office-queue bridge (D-#349, owner ask), branch `feat/english-drive-editor-queue-bridge` (isolated worktree) → PR into dev.** Owner: after WYSIWYG editing, how to send the edited version to the OFFICE print queue? Edited worksheets print LOCALLY (browser); the office queue renders the STORED file. Smoothed the Save-as-PDF → upload path (owner choice, no infra): the editor gains an **"অফিস প্রিন্ট কিউ →"** button + a 2-step hint (Print/preview → Save as PDF → this button → upload the PDF) that jumps to the Print tab's NewPrintRequest (source already defaults to UPLOAD) with the worksheet title prefilled. Reuses the whole existing print-request flow — no server change, no new print code. **NOTE (parallel-session collision):** the main tree was switched to a concurrent session's `feat/hw-lifecycle-redesign` (D-#350) mid-work, whose commit accidentally swept my 2 new STR keys (`edSendToQueue`/`edQueueHint`) onto THAT branch; this change was therefore rebuilt cleanly in an isolated worktree off dev. Flag: dedup those 2 keys when hw-lifecycle lands (append-only STR collision). Gate: app tsc in the worktree pending; expo export pending (CI gates the PR). **Next:** owner tests the bridge on dev.
