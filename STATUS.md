...
_Updated: 2026-06-14 (**CO-2 BUILT — server + app, PR open** — Classroom Observation session footage: `SessionRecording` + the web YouTube-unlisted client uploader (ClassEcho GIS port) + universal auth-free playback (`RecordingPlayer` via Linking). Web-only upload, watch works web+native (unlisted = link-viewable) — D-#202; VOCAB-FREE, reuses `observation:upload` (no new perm). Gate green: jest 1178/1178 (69 suites, +1 [13] over 1165), vocab PASS untouched, shared+server tsc, app tsc + expo web export (754 modules). Not verified live (needs EXPO_PUBLIC_ Google creds; D-#149 handbook privacy check pending). Next = CO-3. Prior: **CT-4-FIX MERGED** — the carried CT-4 dashboard/reports RBAC follow-up: the four read aggregates now gate `authScopes: { authenticated: true }` + resolver gate helpers enforce [`assertChaseAdmin` pattern], so OFFICE can read dashboard/reports per §6/§9 + D-#166 — server-only, vocab-free, NO permission change, **D-#196 [renumbered at merge from #186, which the Finance REQ D-#186–#192 took on main]**. Merged to MAIN (main-direct). Integrated gate green: **jest 1165/1165**, vocab PASS, shared/server tsc. Prior: CM-4 #76, CO-1 #75 [D-#194/#195], CM-3 #74, HR-G1 #73 + APP-FU1 #72, CM-2 #71, CT-5 #70, VC-5, MT #61, HR app. **DEP-1..6 DONE — prod LIVE + dev env + CI/CD.** On main as PRDs (not built): Finance (D-#186–#192), per-user Access-control (D-#193). In flight: CM-5; AC-1 (CO-2 now built — PR open). Carried follow-up: NONE outstanding [CT-4 RBAC now fixed])_

## Now / next
- **Built (Classroom Observation CO-2 — server + app, prd-classroom-observation §CO-2, D-#149 + build ruling D-#202) [branch `worktree-classroom-obs-co2`, PR open]:** session footage — a YouTube-unlisted recording linked to a session anchor, with a web client uploader + universal (auth-free) playback. **Server:** new `SessionRecording {anchor (routineSlotId?, EXACTLY ONE of sectionId|subjectGroupId, subject, teacherId, classDate, periodNumber?), youtubeVideoId (unique), privacyStatus:"unlisted" (model-local literal — VOCAB-FREE), uploadedBy}` (no schoolId, D-#145); `SessionRecordingService` (`recordSessionRecording` + pure `parseYoutubeVideoId` [bare 11-char id OR a pasted YouTube URL] + `getSessionRecording`); resolvers `recordSessionRecording` + `sessionRecording(id)` (both `observation:upload` — Principal/Office, the SAME gate as upload, **NO new permission**) + a ROW-SCOPED nested `recording` field on `ClassroomObservation` (the CO-1 `canReadObservation` rule governs footage reads: observer own / observed teacher at-or-after REVIEWED / P-O all — **non-admins never hit the standalone query → an unlisted id never leaks** beyond an observation they may already read). Audit kind `SESSION_RECORDING_LINKED` in `Audit.ts` (not vocab); **`shared/vocab.ts` + verifier UNTOUCHED** → parallel-safe with CM-5/AC-1; firewall auto-covered by the classroom-observation dir walk (no corpus import). Server stores ONLY the youtubeVideoId — no Google secret in any committed file (§CO-2). **App (APP-ONLY — no server/shared/vocab/contract change):** `lib/youtubeUpload.web.ts` ports the ClassEcho GIS path (token client + GAPI discovery + multipart fetch to the YouTube Data API, `privacyStatus:"unlisted"` + `selfDeclaredMadeForKids:false`, returns the id; creds from `EXPO_PUBLIC_GOOGLE_CLIENT_ID`/`_API_KEY`, never committed) + a native stub (GIS is browser-only → the paste-a-link fallback, which works everywhere); `components/RecordingPlayer.tsx` (`Linking.openURL` the watch URL — plays on web + native, no new dep); `graphql/sessionRecording.ts`; `screens/admin/RecordSessionScreen.tsx` (anchor pickers + dual source: web upload | paste link → recordSessionRecording → plays the saved recording), wired into AdminHome (observation:upload) + AdminStack + BN/EN labels. **Gate GREEN (executed):** vocab verifier PASS (untouched), shared+server tsc clean, **jest 1178/1178** (69 suites; +1 new suite `sessionRecording.test.ts` [13] over the 1165 base); app tsc clean + expo web export (754 modules). **Not verified live** (live upload needs the EXPO_PUBLIC_ Google creds; D-#149 School-Handbook privacy check still pending). **Next = CO-3** (release + teacher response + notify/escalate).
