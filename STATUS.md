...
_Updated: 2026-06-14 (**CM-5 + AC-1 BOTH MERGED to main (main=034a444)** — two server slices landed this cycle, integrated gate green: **jest 1198/1198 [70 suites]**, vocab verifier PASS incl. §C.17, shared/server tsc clean. **CM-5** (PR #78) — Comments: `MeetingComment` + comparison timeline + guardian `childComments`/`childMeetingSlot`, VOCAB-FREE, D-#124 governing + D-#202 ruling; coordinator 3-finder review = no fixes (comparison reads' reps-gate is intentional per PRD §8). **AC-1** (PR #79) — Per-user Access Control: role→template seam swap (`schema.ts` + ALL ~30 production `roleHasPermission(ctx.auth.role,…)` → `callerHasPermission(ctx.auth,…)`), 3 additive `User` fields ZERO-migration, `access:manage` RESERVED-locked, D-#193 + D-#210–#215; sole vocab owner; coordinator 6-finder review = no fixes (byte-identical proof = every prior RBAC test green; verifier §C.17). Merged CM-5 first, AC-1 last (highest blast radius, keep-both append-logs + index/Audit union). **Non-blocking follow-ups recorded:** (1) AC per-user changes apply on the target's NEXT login (JWT-baked, ≤8h TTL — D-#211; relevant to AC-2 UX); (2) `effectivePermissions` recomputes per gate — future per-request memo if profiling warrants; (3) CM-5 comparison timeline is wider than CM-1 section-scope (reps gate, by PRD design). Prior — **CT-4-FIX MERGED** [jest 1165, D-#196]. **DEP-1..6 DONE — prod LIVE + dev env + CI/CD.** On main as PRDs (not built): Finance (D-#186–#192), Saturday-Revision (D-#197–#201). **CO-2 now BUILT — PR #80 open** (server + app, D-#218 ruling [renumbered from D-#202 at merge]; standalone gate green: jest 1178/1178, app tsc + expo export 754; merged with main → re-gated). Next: AC-2 (app editor), CM-6 (Comments app slice), CO-3 (observation release). Carried follow-up: NONE outstanding)_

## Now / next
- **Built (Classroom Observation CO-2 — server + app, prd-classroom-observation §CO-2, D-#149 + build ruling D-#218 [renumbered from D-#202 at merge — CM-5 took D-#202 on main]) [branch `worktree-classroom-obs-co2`, PR #80 open, merged with main + re-gated]:** session footage — a YouTube-unlisted recording linked to a session anchor, with a web client uploader + universal (auth-free) playback. **Server:** new `SessionRecording {anchor (routineSlotId?, EXACTLY ONE of sectionId|subjectGroupId, subject, teacherId, classDate, periodNumber?), youtubeVideoId (unique), privacyStatus:"unlisted" (model-local literal — VOCAB-FREE), uploadedBy}` (no schoolId, D-#145); `SessionRecordingService` (`recordSessionRecording` + pure `parseYoutubeVideoId` [bare 11-char id OR a pasted YouTube URL] + `getSessionRecording`); resolvers `recordSessionRecording` + `sessionRecording(id)` (both `observation:upload` — Principal/Office, the SAME gate as upload, **NO new permission**) + a ROW-SCOPED nested `recording` field on `ClassroomObservation` (the CO-1 `canReadObservation` rule: observer own / observed teacher at-or-after REVIEWED / P-O all — **non-admins never hit the standalone query → an unlisted id never leaks**). Audit kind `SESSION_RECORDING_LINKED` in `Audit.ts` (not vocab); **`shared/vocab.ts` + verifier UNTOUCHED** → was parallel-safe with CM-5/AC-1 (merge = append-log conflicts only, code auto-merged clean); firewall auto-covered by the classroom-observation dir walk. Server stores ONLY the youtubeVideoId — no Google secret committed (§CO-2). **App (APP-ONLY):** `lib/youtubeUpload.web.ts` ports the ClassEcho GIS path (token client + GAPI discovery + multipart fetch, `privacyStatus:"unlisted"` + `selfDeclaredMadeForKids:false`, returns the id; creds from `EXPO_PUBLIC_GOOGLE_CLIENT_ID`/`_API_KEY`, never committed) + a native stub (browser-only → paste-a-link fallback); `components/RecordingPlayer.tsx` (`Linking.openURL` the watch URL — web + native, no dep); `graphql/sessionRecording.ts`; `screens/admin/RecordSessionScreen.tsx` (anchor pickers + dual source upload|paste-link → record → play), wired into AdminHome (observation:upload) + AdminStack + BN/EN labels. **Gate GREEN (standalone, executed):** vocab verifier PASS (untouched), shared+server tsc clean, **jest 1178/1178** (69 suites; +1 suite `sessionRecording.test.ts` [13] over the 1165 base); app tsc clean + expo web export (754 modules). **Re-gate on the merged tree pending in this session** (main brought CM-5 + AC-1's RBAC seam swap). **Not verified live** (live upload needs the EXPO_PUBLIC_ Google creds; D-#149 School-Handbook privacy check still pending). **Next = CO-3** (release + teacher response + notify/escalate).
