...
_Updated: 2026-06-14 (**CM-5 + AC-1 BOTH MERGED to main (main=034a444)** — two server slices landed this cycle, integrated gate green: **jest 1198/1198 [70 suites]**, vocab verifier PASS incl. §C.17, shared/server tsc clean. **CM-5** (PR #78) — Comments: `MeetingComment` + comparison timeline + guardian `childComments`/`childMeetingSlot`, VOCAB-FREE, D-#124 governing + D-#202 ruling; coordinator 3-finder review = no fixes (comparison reads' reps-gate is intentional per PRD §8). **AC-1** (PR #79) — Per-user Access Control: role→template seam swap (`schema.ts` + ALL ~30 production `roleHasPermission(ctx.auth.role,…)` → `callerHasPermission(ctx.auth,…)`), 3 additive `User` fields ZERO-migration, `access:manage` RESERVED-locked, D-#193 + D-#210–#215; sole vocab owner; coordinator 6-finder review = no fixes (byte-identical proof = every prior RBAC test green; verifier §C.17). Merged CM-5 first, AC-1 last (highest blast radius, keep-both append-logs + index/Audit union). **Non-blocking follow-ups recorded:** (1) AC per-user changes apply on the target's NEXT login (JWT-baked, ≤8h TTL — D-#211; relevant to AC-2 UX); (2) `effectivePermissions` recomputes per gate — future per-request memo if profiling warrants; (3) CM-5 comparison timeline is wider than CM-1 section-scope (reps gate, by PRD design). Prior — **CT-4-FIX MERGED** [jest 1165, D-#196]. **DEP-1..6 DONE — prod LIVE + dev env + CI/CD.** On main as PRDs (not built): Finance (D-#186–#192), Saturday-Revision (D-#197–#201). **CO-2 BUILT — PR #80 open** (Classroom Observation session footage, server + app, D-#219 [renumbered from D-#202→#218 at merge]; re-gated on the integrated tree). AC-2 + CM-6 + HR-G2 DONE (Access Control + Comments fully built server + app; HR-G2 staff directory server [jest 1207/1207]; CM guardian meeting-slot card deferred on a server gap; HR-G2 app picker = H8.4 rider later). Next: CO-3 (observation release). Carried follow-up: NONE outstanding)_

## Now / next
- **Built (Classroom Observation CO-2 — server + app, prd-classroom-observation §CO-2, D-#149 + build ruling D-#219 [renumbered from D-#202→#218 at merge — CM-5 took D-#202, HR-G2-build took D-#218]) [branch `worktree-classroom-obs-co2`, PR #80 open, merged origin/main ×3 (CM-5/AC-1 → AC-2 → CM-6/HR-G2) + re-gated]:** session footage — a YouTube-unlisted recording linked to a session anchor, web client uploader + universal (auth-free) playback. **Server:** `SessionRecording {anchor, youtubeVideoId (unique), privacyStatus:"unlisted" (model-local literal — VOCAB-FREE), uploadedBy}`; `SessionRecordingService` (`recordSessionRecording` + pure `parseYoutubeVideoId` + `getSessionRecording`); resolvers `recordSessionRecording` + `sessionRecording(id)` (both `observation:upload`, **NO new permission**) + a ROW-SCOPED nested `recording` field on `ClassroomObservation` (CO-1 `canReadObservation` — non-admins never hit the standalone query → an unlisted id never leaks). Audit `SESSION_RECORDING_LINKED` (Audit.ts, not vocab); **`shared/vocab.ts` + verifier UNTOUCHED**; firewall auto-covered by the dir walk. **App (APP-ONLY):** `lib/youtubeUpload.web.ts` ports the ClassEcho GIS path (token client + multipart fetch with the OAuth bearer token; **client_id-only — `EXPO_PUBLIC_GOOGLE_CLIENT_ID`, NO api key/GAPI/client_secret** [D-#219, refines PRD], never committed) + native stub (paste-a-link fallback); `RecordingPlayer` (`Linking.openURL` watch URL — web + native, no dep); `graphql/sessionRecording.ts`; `RecordSessionScreen` (anchor pickers + dual source upload|paste-link → record → play), wired into AdminHome (observation:upload) + AdminStack + BN/EN labels. **Gate GREEN (executed, merged tree):** vocab verifier PASS (incl. §C.17), shared+server tsc clean, **jest 1220/1220** (72 suites; +1 suite `sessionRecording.test.ts` [13] over the 1207 integrated base); app tsc clean + expo web export (764 modules). **D-#149 privacy gate CONFIRMED by owner 2026-06-14**; `app/.env` set with `EXPO_PUBLIC_GOOGLE_CLIENT_ID` (gitignored) for local dev — **live upload also needs that var in the prod/dev BUILD env (VM)**; playback is auth-free everywhere. **Next = CO-3** (release + teacher response + notify/escalate).
