...
## Now / next
- **In progress (local testing fixes — branch `feat/meaningful-errors-and-hw-lifecycle` off dev, 2026-06-17):**
  Three changes COMMITTED on the branch (not pushed/PR'd yet): (1) **meaningful errors on every screen**
  (`maskError` now surfaces bare `Error` messages too, deny-list keeps Mongo/JWT masked — **D-#259**, extends
  D-#256; server tsc + jest 1472/1472); (2) **homework per-student lifecycle screen** (`HomeworkRecordsScreen`
  wires the missing GIVEN→DUE→SUBMITTED step so the Checking queue is reachable; app tsc + expo export); (3)
  **stale unique-index migration** (`migrate-hw-record-index.ts` — dropped `hwItemId_1_studentId_1` on
  scdhub_local; **dev/prod still need it run** before the Wrong→resubmission path is used). **NEXT: working
  through the live-testing follow-up backlog** (in the agent memory file `project_local_testing_followups.md`):
  bulk reviewer-assignment + Principal assignment-count overview (owner request); i18n module-level-`STR`
  capture sweep; homework day-tally refetch-on-focus; Checking-queue student names; "Section"→"Class" display
  post-merge; routine slot class-name + nested-screen hamburger. Not pushed/PR'd — owner driving order.
