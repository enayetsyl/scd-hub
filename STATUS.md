...
- 2026-07-09: **PLANNED (docs only) — Print queue (D-#281, PRD `docs/prd-print-queue.md`).** **D-#280:** the Today card gains an amber countdown to prepare the assignment question, targeting **07:00 on the RESOLVED delivery date** (holiday-rolled), shown from the delivery week's start, vanishing the instant the item is delivered, and flipping to the existing red `assignment_entry` alert once overdue. **D-#281:** one `PrintRequest` queue for everything the Office prints — sources `setId` XOR `contentArtifactId` XOR `fileIds[]` (new `print_upload` StoredFile kind) XOR `linkUrl`; statuses `REQUESTED → PRINTED → DELIVERED` (+`CANCELLED`) — the `DELIVERED` state is the only genuinely new one; the class-test print queue is **absorbed** (`ClassTest.printRequestId`, CT queue retired); **no new permission** (submit `tracker:write`, operate `roster:manage`); **no PDF snapshot needed** (an assembled set is locked, so `setId` is already immutable). Slices PQ-1..PQ-5. **NEITHER IS BUILT.** **Next:** land the attendance branch first (below), then build.
