...
- **Built (FINANCE MODULE COMPLETE — FIN-1..FIN-6, server + app) [branch `claude/open-prds-o1bwkh`]:** the whole
  ...
  ruling D-#230** — the split is a derived memo, no double-count). **FIN-3** Qard/IOU register (`FinanceParty` +
  ...
  `finance.fee_due.chase.*` MT keys + `FINANCE_PARTY_KINDS` + `RECON_SOURCES` + `BUDGET_LINE_KINDS`. **D-#230 newly
