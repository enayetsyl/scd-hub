...
- 2026-07-29 (cont. 2): **LIVE-VERIFIED the Exams screens against a running stack — 4 defects found and fixed, branch `feat/exams` → PR #424.** Ran the `verify` skill properly: server on **:4055** (own PORT so it cannot collide with the shared checkout's session) against **`scdhub_local`** (the isolated test copy — checked the databaseName before writing anything), expo web on :8090, Playwright chromium. **The headline: `ExamMarkGrid` was broken for every paper and neither tsc, jest, nor the web export could see it** — the screen resolved its paper out of `examPapers(examId)` while the hub navigates with only a `paperId`, so the query stayed paused and the screen rendered a bare error. Fixed with a new `examPaper(paperId)` read so the screen stands alone (deep link / notification / duty row), and the redundant route param removed. Also fixed: an out-of-range mark previewed an impossible converted value ("রূপান্তরিত ১২০ / ৮০") with Save enabled → inline range error + Save disabled; `ExamReportCard` was registered but **unreachable** → per-roster-row entry point; the custody all-events list said "no exceptions" when it meant "no handovers"; GPA printed in Western digits on an otherwise Bangla-numeral card. **Seen working on screen:** 150/200 → ৬৭.৫ and 77/100 → ৬১.৫ (nearest-0.5, one shared helper across screen/server/PDF); custody board `দেওয়া ১৪ · গোনা ০ · অপেক্ষমাণ ১` with `উপস্থিত শিক্ষার্থী` derived from marks not typed; a report card printing `১৪৯.৫/৩০০ · ০.০০ · F` with the any-F explanation naming ইংরেজি; and **গণিত rendering with no CT row at all — D-#376 visible in the UI**. API probe also proved the sum≠100 guard, over-scale refusal, CT-pull refusal on a no-CT paper, blank-not-zero CT proposals, self-handover + giver-self-ack refusals, tabulation blocked on missing marks, submit blocked on un-tabulated papers. **Gate:** shared build; shared+server+app tsc; vocab verifier PASS; 2487/2487 Jest (146 suites); `expo export --platform web` exit 0. *Process note:* the `verify` skill's TEACHER login (`teacher@scd.test`) does not exist in this DB — only the PRINCIPAL account worked, so the receiver-side acknowledge flow was exercised via jest + API, not the UI. **Now / next:** the acknowledge-as-receiver UI path and the guardian-facing published card are still undriven; EX-10 backfill remains unbuilt.

- 2026-07-29 (cont. 1): **BUILT — the Exams APP SCREENS on top of the EX-1..EX-9 server contract, branch `feat/exams` (same worktree) → PR #424.** New `ExamsTab` gated on `exam:read`, five screens: exam hub (duty-scoped for a teacher, full + submit/approve for Office/Principal, custody inbox pinned top), mark grid (roster in printed schoolId order, FINAL on the script's scale with the nearest-0.5 preview computed by the SAME shared helper as server + PDF), rechecker worksheet (the hidden checker column renders as `•••` — the screen never fetches it another way), custody board (waiting-on-you, derived per-stage balance, handover form, exceptions; acknowledging asks for YOUR count and blocks a mismatch without a reason), and the report-card preview (derived only — the screen never recomputes a GPA). Two server reads added rather than letting screens guess: `examPaperRoster` and `examCustodyRecipients` (the latter because `users` needs `user:manage`, which an Office clerk recording a handover does not hold — a real gate bug avoided). **`navInitialRoute.test.ts` extended** to cover all four param-requiring exam screens and to assert the Exams stack opens on the param-less hub — the D-#357 trap that took down five tabs in production, which tsc cannot see. **Gate (executed):** shared build; shared + server + app `tsc --noEmit`; vocab verifier PASS; **2487/2487 server Jest across 146 suites**; `expo export --platform web` **exit 0** with the full output inspected (a `| tail` pipe would have reported tail's exit code, not the bundler's). **Now / next:** live-verify the screens against the dev site once PR #424 merges; EX-10 backfill still unbuilt (owner deferred); no e2e/Playwright drive of the exam flow yet.

- 2026-07-29: **BUILT — Exams module server contract, EX-1..EX-9 (D-#375–#382), branch `feat/exams` (isolated worktree `exams` off `origin/dev`) → PR into dev.** The whole exam cycle now lives in the app: exam + per-paper composition + per-exam grade scale (EX-1), duty assignment with the checker≠rechecker guard (EX-2), mark entry with paper-scale conversion and the CT pull (EX-3), an INDEPENDENT recheck with divergence resolution and a tabulation lock (EX-4), the derived report card (EX-5), the **two-signature custody chain** the owner actually asked for (EX-6/7), the custody board + exceptions + notifications (EX-8), and submit→approve→publish with the report-card PDF (EX-9). **Server-side only — no app screens yet; EX-10 backfill deliberately not built** (owner: "without backfilling"). Load-bearing rulings that shaped the code, not just the docs: composition is PER PAPER (a per-class-band lookup would have written a silent 0 CT into ~16 Class-3 Maths cards); MISSING IS NEVER ZERO (no class-test history pulls blank, an absent component is null); a count mismatch is DISPUTED holding BOTH numbers rather than one overwriting the other; and the custody chain BLOCKS tabulation, so the physical count reaches all the way to the guardian's screen. Reused rather than rebuilt: `PrintRequest` (D-#281), the CT-8/CO-8 publish shape, the class-test tracker as the CT source, `assertGuardianOfStudent` for the PDF route's guardian gate (a hand-rolled GuardianLink lookup was caught missing the `active` check), and `resolveSubjectTeacher` for the Adab gate. **Gate (executed this session):** shared build green; shared + server + app `tsc --noEmit` green; vocab verifier PASS; `npm run test --workspace=server` **2486/2486 green across 146 suites** (baseline on dev was 2326/140 → +160 tests, +6 suites, no regressions). The acceptance suite reproduces the real 2026 cards from the owner's PDFs, including Rehana Bint Mustafa printing 0.00/F at 552/800 on a single Maths F — the rule no total-based formula reproduces. **Now / next:** app screens for the exam workspace (roster mark grid, rechecker worksheet, custody board, report-card preview) — none exist yet; then EX-10 backfill when the owner wants the 2026 history loaded.
