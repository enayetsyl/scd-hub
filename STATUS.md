...
- 2026-07-25 (cont. 3): **PROMOTED SP-1..SP-4 to prod, then HOTFIXED a production regression I caused.** PRs: #377 (feature → dev), #378 (dev → main, promoted at owner instruction WITHOUT the live drive), #380 (hotfix → dev), #381 (hotfix → main). **The regression:** SP-3 registered `<Stack.Screen name="StudentProfile">` as the FIRST screen of the Homework / Assignment / Attendance / Class Test / Admin navigators. React Navigation treats a stack’s first screen as its **initial route**, so all five tabs mounted the profile instead of their home screen, with `params === undefined`, and the screen’s direct `useRoute().params` destructure threw → error boundary → "Something went wrong" on five tabs **in production**. **Why every gate stayed green:** the param type declares `studentId` required and all call sites pass it (tsc clean), the bundle builds (expo export clean), and server jest never renders the app. Only opening the tab reveals it — the exact failure the skipped live drive existed to catch. **Fixes (all shipped):** (a) StudentProfile moved LAST in all five navigators; (b) the screen no longer trusts params — `?? {}`, hooks kept unconditional, queries paused without a studentId, empty state instead of a throw, so a recurrence is cosmetic not fatal; (c) `server/src/__tests__/navInitialRoute.test.ts` encodes the invariant (server suite because CI gates it and the app workspace has no test runner) — proven honest by reintroducing the bug in one navigator and watching it fail by name, and its parser-sanity assertion immediately caught a first-attempt line-based parser that silently read multi-line `<Stack.Screen>` tags as empty navigators. Gate: app tsc; expo web export exit 0 (full log); **136 suites / 2221 jest**; vocab verifier PASS. **STILL OWED:** the live drive itself — walk the profile as a full-view class teacher, a narrowed subject teacher and a Principal, and eyeball one rendered PDF (Bangla glyphs, narrowing note, printed-by footer). Lesson recorded in memory as `rn-initial-route-param-trap`: a green CI gate plus a successful deploy is NOT evidence the app works.
