...
import { Screen, Card, Body, Muted, Badge, Chip, Notice } from "../../components/ui";
...
export default function VocabReportScreen({ route }: Props): React.ReactElement {
  ...
  const [reportQ, refetchReport] = useQuery({ query: VOCAB_TEST_REPORT_QUERY, variables: { testId } });
  ...
  return (
    <Screen padded={false}>
      <ScrollView contentContainerStyle={{ padding: space(4) }}>
        <QueryGate
          result={reportQ}
          onRetry={() => refetchReport({ requestPolicy: "network-only" })}
