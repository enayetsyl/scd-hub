...
import { Screen, H2, Body, Muted, Card, Button, Field, Select, Divider, EmptyState, Loader } from "../../components/ui";
...
export default function ScopeGrantScreen(_props: Props): React.ReactElement {
  ...
  async function onAssign(): Promise<void> {
    ...
    if (Number.isNaN(days) || days < 1) {
      toast.show(STR.errGeneric, "danger");
      return;
    }
    let startIso: string;
    if (startDate.trim()) {
    ...
    toast.show(STR.grantCreated, "ok");
    reloadGrants();
  }
